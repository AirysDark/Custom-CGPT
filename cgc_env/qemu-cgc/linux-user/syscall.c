...
static abi_long do_receive(abi_long fd, abi_ulong buf, abi_long count, abi_ulong p_rx_bytes) {
    ...
    if (count < req_count)
        fprintf(stderr, "FOR_CGC: Pre-shortening receive count=%ld to %ld\n", req_count, count);
#endif
    ...
    if (count != 0) {
        ...
        if (ret >= 0) {
            ...
            if (receive_count_fp) {
                fprintf(receive_count_fp, "%lu %lu\n", ret, count);
            }
...
#ifdef DEBUG_LENIENT_LENGTHS
    if (count < req_count)
        fprintf(stderr, "FOR_CGC: Pre-shortening transmit count=%ld to %ld\n", req_count, count);
#endif
...
#ifdef DEBUG_LENIENT_LENGTHS
    if (count < req_count)
        fprintf(stderr, "FOR_CGC: Pre-shortening random() count=%ld to %ld\n", req_count, count);
#endif
                    ...
                    abi_long arg8)
{
    ...
    default:
        gemu_log("qemu: Unsupported syscall: %ld\n", num);
        ret = TARGET_ENOSYS;
        break;
    }
#ifdef DEBUG
    ...
    if (!((ret >= 0) && (ret <= 6))) { /* CGC syscalls return either 0 or an error */
        fprintf(stderr, "qemu: INTERNAL ERROR: syscall %ld tried to return %ld, but all CGC syscall return either 0 or one of the CGC_Exxx (positive) values.\n", num, ret);
        exit(-33);
    }
    return ret;
}
