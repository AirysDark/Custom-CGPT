                       ...
                       uint32_t next_eip)
{
    ...
    /* check that env->rip is in the CS segment limits */
...
void helper_syscall(CPUX86State *env, int next_eip_addend)
{
    ...
    env->exception_next_eip = env->rip + next_eip_addend;
    cpu_loop_exit(cs);
}
#else
void helper_syscall(CPUX86State *env, int next_eip_addend)
{
    ...
    if (env->hflags & HF_LMA_MASK) {
        ...
        env->regs[R_ECX] = env->rip + next_eip_addend;
        ...
        env->regs[R_ECX] = (uint32_t)(env->rip + next_eip_addend);
    ...
    if (env->hflags & HF_LMA_MASK) {
        ...
        if (dflag == 2) {
            ...
            cpu_x86_load_seg_cache(env, R_CS, selector | 3,
                                   ...
                                   DESC_S_MASK | (3 << DESC_DPL_SHIFT) |
                                   DESC_CS_MASK | DESC_R_MASK | DESC_A_MASK);
            env->rip = (uint32_t)env->regs[R_ECX];
        }
 ...
 * the int instruction. next_eip is the env->rip value AFTER the interrupt
    ...
    if (qemu_loglevel_mask(CPU_LOG_INT)) {
        if ((env->cr[0] & CR0_PE_MASK)) {
            ...
            qemu_log("%6d: v=%02x e=%04x i=%d cpl=%d IP=%04x:" TARGET_FMT_lx
                     ...
                     env->segs[R_CS].selector, env->rip,
                     (int)env->segs[R_CS].base + env->rip,
...
#if 0
            {
                ...
                qemu_log("       code=");
                ptr = env->segs[R_CS].base + env->rip;
                for (i = 0; i < 16; i++) {
                           ...
                           int next_eip_addend)
{
    ...
    if (e2 & DESC_S_MASK) {
        ...
        case 5: /* task gate */
            ...
            next_eip = env->rip + next_eip_addend;
