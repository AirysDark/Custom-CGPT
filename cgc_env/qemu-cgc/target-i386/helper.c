                        ...
                        int flags)
{
    ...
    if (flags & CPU_DUMP_CODE) {
        target_ulong base = env->segs[R_CS].base + env->rip;
        target_ulong offs = MIN(env->rip, DUMP_CODE_BYTES_BACKWARD);
        uint8_t code;
        char codestr[3];
                             ...
                             int is_write1, int mmu_idx)
{
    ...
    printf("MMU fault: addr=%" VADDR_PRIx " w=%d u=%d eip=" TARGET_FMT_lx "\n",
           addr, is_write1, is_user, env->rip);
#endif
...
void cpu_report_tpr_access(CPUX86State *env, TPRAccess access)
{
    ...
    if (kvm_enabled()) {
        ...
        apic_handle_tpr_access_report(cpu->apic_state, env->rip, access);
    }
}
