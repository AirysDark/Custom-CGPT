...
void do_smm_enter(X86CPU *cpu)
{
    ...
    stq_phys(cs->as, sm_state + 0x7f78, env->rip);
    ...
    stl_phys(cs->as, sm_state + 0x7ff0, env->rip);
...
void helper_rsm(CPUX86State *env)
{
    ...
    env->rip = ldq_phys(cs->as, sm_state + 0x7f78);
    ...
    cpu_load_eflags(env, ldl_phys(cs->as, sm_state + 0x7ff4),
                    ~(CC_O | CC_S | CC_Z | CC_A | CC_P | CC_C | DF_MASK));
    env->rip = ldl_phys(cs->as, sm_state + 0x7ff0);
